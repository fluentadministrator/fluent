...
size_t FluentInterpreter::handleLet(const std::vector<std::string>& tok, size_t idx) {
    ...
    FluentValue result = resolveValue(trim(valStr));
    // If already declared global, keep it in globals_
    if (globals_.count(name)) {
        result.is_global = true;
        globals_[name] = result;
    } else {
        setVar(name, result);
    }
    return idx + 1;
...
size_t FluentInterpreter::handleMake(const std::vector<std::string>& tok, size_t idx) {
    ...
    if (tok.size() >= 3 && tok[2] == "global") {
        // Move existing value to globals, or create empty placeholder
        FluentValue val;
        if (vars_.count(name)) { val = vars_[name]; vars_.erase(name); }
        else if (globals_.count(name)) val = globals_[name];
        val.is_global = true;
        globals_[name] = val;
        return idx + 1;
    }
...
size_t FluentInterpreter::handleImport(const std::vector<std::string>& tok, size_t idx) {
    ...
    if (fs::exists(name + ".fluent")) {
        ...
        sub.run();
        // Export everything marked global
        for (auto& [k,v] : sub.globals_) globals_[k] = v;
        // Also export regular vars that were declared global (make X global before let X be)
        // by checking vars_ that have is_global flag set
        for (auto& [k,v] : sub.vars_) {
            if (v.is_global) globals_[k] = v;
        }
        modules_[name] = {};
