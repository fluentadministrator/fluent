...
void FluentInterpreter::loadScript() {
    ...
    // Pre-fetch hardware info in background so it's ready instantly when needed
    std::thread([this]() {
        for (auto& k : {"os","cpu","ram","gpu","storage","vram"})
            getHardwareInfo(k);
    }).detach();
}
...
static std::unordered_map<std::string, std::string> hw_cache;
static std::mutex hw_cache_mutex;

std::string FluentInterpreter::getHardwareInfo(const std::string& key) {
    {
        std::lock_guard<std::mutex> lock(hw_cache_mutex);
        auto it = hw_cache.find(key);
        if (it != hw_cache.end()) return it->second;
    }
    ...
    if (key == "all")     { std::string r = getHardwareInfo("os") + " | " + getHardwareInfo("cpu"); { std::lock_guard<std::mutex> lock(hw_cache_mutex); hw_cache["all"] = r; } return r; }
    ...
    if (key == "all")     { std::string r = getHardwareInfo("os") + " | " + getHardwareInfo("cpu") + " | " + getHardwareInfo("ram"); { std::lock_guard<std::mutex> lock(hw_cache_mutex); hw_cache["all"] = r; } return r; }
#endif
